...
# DQN Agent
class PERDuelingDoubleDeepNStepSarsa:
    def __init__(self, env, exploration, n_step,
        ...
        self.n_next_actions = np.zeros((n_step,), dtype=np.int64)
    ...
    def optimize(self):
        ...
        states, actions, rewards, next_states, next_actions, dones, indices, weights = self.buffer.sample(self.batch_size)
        ...
        next_actions = torch.tensor(next_actions, dtype=torch.int64, device=self.device).unsqueeze(1)
       ...
       # Compute Double Q-Learning target Q values
        with torch.no_grad():
            # Use target network to evaluate the Q-value of the nest action
            next_q_values = self.target_net(next_observations).gather(1, next_actions)
        ...
        for episode in range(num_episodes):
            ...
            while True:
                if t < T:
                    ...
                    self.n_next_actions[idx] = next_action
                ...
                if tau >= 0:
                    ...
                    sum_rs = 0
                    last_i = min(tau + self.n_step, T) - 1
                    for i in range(last_i, tau - 1, -1):
                      index = i % self.n_step
                      sum_rs = self.gamma * sum_rs + self.n_rewards[index]
                    index = last_i % self.n_step
                    next_s = self.n_next_states[index]
                    next_a = self.n_next_actions[index]
                    d = self.n_dones[index]
                    # Store in replay buffer
                    self.buffer.push(s, a, sum_rs, next_s, next_a, d)
