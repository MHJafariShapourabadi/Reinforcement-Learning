...
class ActorCriticEligibilityTraces:
    ...
    actor_lambda_trace=0.9, critic_lambda_trace=0.9,
    ...
    gamma=0.99, seed=None, verbose=False):
        ...
        self.actor_lambda_trace = actor_lambda_trace
        self.critic_lambda_trace = critic_lambda_trace
        ...
        self.actor_trace = {name: torch.zeros_like(param).to(self.device) for name, param in self.actor.named_parameters()}
        self.critic_trace = {name: torch.zeros_like(param).to(self.device) for name, param in self.critic.named_parameters()}
    ...
    def reset_traces(self):
        for name in self.actor_trace:
            self.actor_trace[name].zero_()
        for name in self.critic_trace:
            self.critic_trace[name].zero_()
    ...
    def train(self, max_episodes):
        ...
        for episode in range(max_episodes):
            ...
            while True:
                ...
                delta = (target - state_value.detach()).clamp(min=-self.Huberbeta, max=self.Huberbeta).div(self.Huberbeta)

                self.critic_optimizer.zero_grad()
                state_value.backward()
                # Update eligibility traces
                for name, param in self.critic.named_parameters():
                    self.critic_trace[name] = self.gamma * self.critic_lambda_trace * self.critic_trace[name] + param.grad
                # Update gradients
                for name, param in self.critic.named_parameters():
                    param.grad_ = delta *  self.critic_trace[name]
                ...
                actor_loss = -(I * log_prob)
                actor_loss.backward()
                # actor_loss.backward(retain_graph=True)
                # Update eligibility traces
                for name, param in self.actor.named_parameters():
                    self.actor_trace[name] = self.gamma * self.actor_lambda_trace * self.actor_trace[name] + param.grad
                # Update gradients
                for name, param in self.actor.named_parameters():
                    param.grad_ = advantage * self.actor_trace[name]
                # # add entropy gradient to gradients
