...
class PPOGAEROLL:
    def __init__(self, env, n_rollout, lambd = 0.9,
    ...
    n_updates_per_iter=5, batch_size=64,
    ...
    gamma=0.99, max_grad_norm=0.5, target_kl = 0.02, seed=None, verbose=False):
        ...
        self.buffer = ReplayBuffer(capacity=self.n_rollout * self.env.n_envs, n_envs=self.env.n_envs,
    ...
    def train(self, max_episodes):
        ...
        n_states = SpecialDeque(maxlen=self.n_rollout)
        n_values = SpecialDeque(maxlen=self.n_rollout)
        n_next_values = SpecialDeque(maxlen=self.n_rollout)
        n_actions = SpecialDeque(maxlen=self.n_rollout)
        n_log_probs = SpecialDeque(maxlen=self.n_rollout)
        n_rewards = SpecialDeque(maxlen=self.n_rollout)
        n_I = SpecialDeque(maxlen=self.n_rollout)
        n_terminateds = SpecialDeque(maxlen=self.n_rollout)
        n_dones = SpecialDeque(maxlen=self.n_rollout)
        ...
        for t in count(1):
            ...
            if t % self.n_rollout == 0:
                # print(f"enter t: {t}")
                advantages = 0.0
                for i in reversed(range(self.n_rollout)):
                    ...
                    targets = advantages + n_values[i].detach().clone()

                    self.buffer.push(n_I[i].detach().cpu(), n_states[i].detach().cpu(), n_values[i].detach().cpu(), n_actions[i].detach().cpu(), n_log_probs[i].detach().cpu(), targets.detach().cpu())
                
                if self.buffer.full():
                    # print(f"buffer full t: {t}")
                    for _ in range(self.n_updates_per_iter):
                        self.n_updates += 1
                        # Sample from replay buffer with PER
                        for mb_I, mb_states, mb_states_value_old, mb_actions, mb_log_probs_old, mb_targets in self.buffer.sample(self.batch_size):
