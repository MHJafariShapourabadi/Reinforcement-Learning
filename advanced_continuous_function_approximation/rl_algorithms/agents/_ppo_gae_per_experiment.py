...
class Actor(nn.Module):
    def __init__(self, state_dim, num_actions):
        ...
        self.fc_mean = nn.Linear(64, num_actions)
        self.fc_log_std = nn.Linear(64, num_actions)
    ...
    def forward(self, x):
        # print(f"x1: {x}")
        x = self.fc1(x)
        # print(f"fc1 wieghts: {self.fc1.weight}")
        # print(f"x2: {x}")
        ...
        # print(f"means in net: {means}")
        # print(f"stds in net: {stds}")
        return means, stds
...
class Critic(nn.Module):
    ...
    def forward(self, x):
        ...
        value = self.fc3(x).squeeze(-1)  # Ensure scalar output
        return value
...
class PPOGAEPER:
    ...
    gamma=0.99, max_grad_norm=0.5, seed=None, verbose=False):
    ...
    def select_action(self, state, info):
        ...
        with torch.no_grad():
            ...
            mean, std = mean.squeeze(0).detach(), std.squeeze(0).detach()
            if self.is_training:
                dist = Independent(Normal(mean, std), 1)
                raw_action = dist.sample()
                action = torch.tanh(raw_action).detach().cpu().numpy() * self.action_upper_bound.item()
                # action = torch.clamp(raw_action, self.action_lower_bound, self.action_upper_bound).detach().cpu().numpy()
            else:
                action = torch.tanh(mean).detach().cpu().numpy()
        return action

    def _corrected_log_prob(self, actions_dist, raw_actions):
        """
        Correct log prob after tanh squashing (for PPO):
        log pi(a) = log N(a_raw) - sum(log(1 - tanh(a_raw)^2))
        """
        # log_probs_raw = actions_dist.log_prob(raw_actions)
        # corrections = torch.sum(
        #     2 * (torch.log(torch.tensor(2.0, device=self.device)) - raw_actions - F.softplus(-2.0 * raw_actions)),
        #     dim=1
        # )
        # return log_probs_raw - corrections
        ...
        # This accounts for the change in volume due to the tanh transformation
        log_det_jacobian = torch.sum(torch.log(self.action_upper_bound.item() * (1 - torch.tanh(raw_actions) ** 2 + epsilon)), dim=-1)

        # Compute the corrected log probability of the squashed action
        log_probs = log_probs_raw - log_det_jacobian
    ...
    def train(self, max_episodes):
        ...
        for t in count():
            ...
            # raw_actions = actions_dist.rsample()  # for reparameterization trick (PPO needs this)
            raw_actions = actions_dist.sample().detach()
            actions = torch.tanh(raw_actions) * self.action_upper_bound.item()
            log_probs = self._corrected_log_prob(actions_dist, raw_actions)
            # actions = torch.clamp(raw_actions, self.action_lower_bound, self.action_upper_bound)
            ...
            n_actions.append(raw_actions.detach())
            ...
            if t >= self.n_step - 1:
                ...
                if len(self.buffer) > self.batch_size:
                    ...
                    mb_I, mb_states, mb_states_value_old, mb_raw_actions, mb_log_probs_old, mb_targets, indices, weights = self.buffer.sample(self.batch_size)
                    mb_I, mb_states, mb_states_value_old, mb_raw_actions, mb_log_probs_old, mb_targets, weights = mb_I.to(self.device), mb_states.to(self.device), mb_states_value_old.to(self.device), mb_raw_actions.to(self.device), mb_log_probs_old.to(self.device), mb_targets.to(self.device), weights.to(self.device)
                    ...
                    mb_means, mb_stds = self.actor(mb_states)
                    mb_actions_dist = Independent(Normal(mb_means, mb_stds), 1)
                    mb_log_probs = self._corrected_log_prob(mb_actions_dist, mb_raw_actions)
            ...
            for worker_id in range(len(infos)):
                ...
                if info['done']:
                    ...
                    print(f"Episodes {episodes}, actor avg loss: {actor_loss_avg : .5f}, critic avg loss:{critic_loss_avg : .5f}, Reward: {episode_reward : .5f}, Steps: {steps_in_episode}, actor lr: {self.actor_scheduler.get_last_lr()[0] : .5f}, critic lr: {self.critic_scheduler.get_last_lr()[0] : .5f}")
