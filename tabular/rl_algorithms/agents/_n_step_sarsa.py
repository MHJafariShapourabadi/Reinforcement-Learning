...
class NStepSarsa(Agent):
    def __init__(self, env, learning_rate, gamma, n_step=1, learning_rate_dacay=0.0, initial_qtable=None, seed=None):
        super(NStepSarsa, self).__init__(env=env, learning_rate=learning_rate, gamma=gamma, learning_rate_dacay=learning_rate_dacay, initial_qtable=initial_qtable)
        self.n_step = n_step
    ...
    def update(self, step, T):
        ...
        if t >= 0:
            ...
            if t + self.n_step < T:
                ...
                G = G + (self.gamma ** self.n_step) * self.qtable[s, a]
