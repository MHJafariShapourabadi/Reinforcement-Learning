        ...
        while True:
            ...
            if cmd == "reset":
                ...
                if done:
                    ...
                    episode_reward = 0
                    obs, info = env.reset()
                    rew, terminated, truncated = 0, False, False
                    I_buf[idx] = 0
                else:                    
                    act = int(act_buf[idx])
...
class AsyncVectorEnv:
    ...
    """
    def __init__(self, env_fns, env_fns_args, obs_dtype=np.float32, act_dtype=np.int64):
        ...
        self.act_shape = (self.n_envs,)
        self.act_dtype = np.dtype(act_dtype)
        nbytes_act = np.prod(self.act_shape) * self.act_dtype.itemsize
        self.shm_act = shared_memory.SharedMemory(create=True, size=int(nbytes_act))
        self.act_buf = np.ndarray(self.act_shape, dtype=self.act_dtype, buffer=self.shm_act.buf)
        ...
        for worker_id, (w_remote, remote, fn, args) in enumerate(zip(self.work_remotes, self.remotes, env_fns, env_fns_args)):
            args = (w_remote, remote, fn, args,
                    ...
                    self.shm_act.name, self.act_shape, self.act_dtype,
